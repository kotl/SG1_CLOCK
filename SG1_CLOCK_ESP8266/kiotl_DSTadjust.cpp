...
simpleDSTadjust::simpleDSTadjust(
    week_t s_week, dow_t s_dow, month_t s_month, uint8_t s_hour, int s_offset,
    week_t e_week, dow_t e_dow, month_t e_month, uint8_t e_hour, int e_offset) {
  struct dstRule s = {"", s_week, s_dow, s_month, s_hour, s_offset};
  struct dstRule e = {"", e_week, e_dow, e_month, e_hour, e_offset}; 
  dstStartRule = s;
  dstEndRule = e;
}
